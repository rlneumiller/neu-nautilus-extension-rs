...
use crate::nautilus4_ffi::nautilus_file_info_invalidate_extension_info;
use crate::nautilus4_ffi::{
...
macro_rules! info_provider_iface {
    ($iface_init_fn:ident, $update_file_info_fn:ident, $update_file_info_bg_fn:ident, $cancel_update_fn:ident, $rust_provider:ident, $set_rust_provider:ident) => {
        ...
        pub unsafe extern "C" fn $iface_init_fn(iface: gpointer, _: gpointer) {
            use crate::nautilus4_ffi::NautilusInfoProviderIface;
        ...
        pub unsafe extern "C" fn $update_file_info_fn(
            ...
            let closure_copy = unsafe { g_closure_ref(update_complete) };
            unsafe { *handle = mem::transmute(Box::into_raw(Box::new(my_handle_local))) };
            tx.send((
                Box::new(file_info),
                unsafe { &mut *provider },
                unsafe { &mut *closure_copy },
                unsafe { &mut **handle },
            ))
            .unwrap();
        ...
        pub unsafe extern "C" fn $cancel_update_fn(
            ...
            let mut my_handle = unsafe { (*handle).lock().unwrap() };
        ...
        fn $update_file_info_bg_fn(
            ...
            use crate::nautilus4_ffi::nautilus_info_provider_update_complete_invoke;
        ...
        static $rust_provider: LazyLock<Mutex<Option<Box<dyn InfoProvider>>>> =
