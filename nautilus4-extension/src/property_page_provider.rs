...
use crate::nautilus4_ffi::{nautilus_property_page_new, NautilusPropertyPageProvider};
...
macro_rules! property_page_provider_iface {
    ($iface_init_fn:ident, $get_pages_fn:ident, $rust_provider:ident, $set_rust_provider:ident) => {
        ...
        pub unsafe extern "C" fn $iface_init_fn(iface: gpointer, _: gpointer) {
            use crate::nautilus4_ffi::NautilusPropertyPageProviderIface;

            let iface_struct = iface as *mut NautilusPropertyPageProviderIface;
            unsafe {
                (*iface_struct).get_pages = Some($get_pages_fn as unsafe extern "C" fn(*mut NautilusPropertyPageProvider, *mut GList) -> *mut GList);
            }
        }

        pub unsafe extern "C" fn $get_pages_fn(
        ...
        static $rust_provider: LazyLock<Mutex<Option<Box<dyn PropertyPageProvider>>>> =
